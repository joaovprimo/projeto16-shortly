...
export async function getRanking (req,res){
    ...
    try{
        ...
        SELECT users.id AS id, users.name AS name, COUNT(urls.id) AS "linksCount", SUM(urls."visitCount") AS "visitCount" FROM users LEFT JOIN urls ON users.id = urls."userId" GROUP BY users.id ORDER BY "visitCount" DESC LIMIT 10;
        `)
